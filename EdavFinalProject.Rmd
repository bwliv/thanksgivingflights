...
The authors of this problem experienced this firsthand during Thanksgiving 2019. During a Wednesday train trip from New York to Philadelphia, the American of the group told the rest of the group, "Penn Station is going to be a mob scene." 
...
Even stranger, 45 minutes later, Penn Station had allegedly become a [total mob scene](https://twitter.com/JamesMartinSJ/status/1199761379584032769). Just like that. 
...
Air travel is an especially interesting beast to tackle, because there are so many options. If you're driving from New York to Boston, you take I-95. If you're taking the train from Richmond to D.C., you take Amtrak. But when you're choosing your flights, there are so many more options in terms of airlines, local airports, and connecting airports, on top of the time old-question of when to leave to "beat the rush."
...
However, when we *count* delays, we will only include flights delayed by more than 15 minutes, which adheres to the FAA's definition of a delay.
...
There are a plethora of variables, including different types of delay times, taxi times, flight times, origin, destination, etc. There's much more than we needed here. We will consider a total of 1.6 million flights across these analyses. 
...
It seems that, outside of one flight that was missing a curiously large amount of data, the only missing data is for weather delays or carirer delays - which we will asume were labeled NAs when that delay category did not apply to a flight. Outside of that, it seems that NAs shouldn't be a major barrier to our analysis.

With that in mind, we will mostly pass over NAs (i.e. not include them in means) when computing our statistics, and keep this structure in mind when we consider delay types later.

### V. Results

Without any further ado, we will now present our research. Happy flying!

## What Airline Is Best For Thanksgiving?
...
Among the three major legacy mainline carriers, Delta has the fewest delays, followed by American Airlines, and United performs by far the worst. Southwest performs a little better than American Airlines, and Alaska Airlines (AS) performs a little worse than American Airlines.
...
2. Regional or heavily region-specific airlines (regional airlines would be contracted carriers such as Mesa, heavily region-focused airlines would be like Hawaiian Airlines).
3. Inactive airlines
...
We will present these airlines graphically sorted from the largest airline to the smallest (as they are above), and keep this consistent throughout the presentation.
...
The mysterious 2018 mess had a much bigger impact on the Midwest, Northeast, and South than it did on the West. This suggests that it was probably weather-related, but we'll explore that later. 

Outside of this, it seems that delays are somewhat pocketed to different regions, but it seems that there are ripple effects that preclude a truly regional analysis.
...
There are a lot of things going on here, but it's fairly clear from this graph that airline performance does vary by region. For instance, Frontier is much worse than United for flights leaving from the South, but United performs worse when it comes to flights leaving from the Midwest (although this could be due to the 2018 mess we'll, again, discuss later).

We see that in the South, Delta and Southwest Airlines have the best performance, followed by American Airlines, which is intuitive as they all have giant hubs in that region. 
...
theme_dotplot <- theme_bw(14) +
    theme(axis.title = element_text(size=12), axis.text.y = element_text(size = rel(.75)),
...
The worst-affected airports are the Rapid City, SD (RAP), Santa Barbara, CA (SBA), Fargo, ND (FAR).

Of course, many of these are small airports, so probably either (a) you're forced to fly out of them to get where you're going or (b) they're of no relevance to you because you can't change planes there.

Thus, we will focus our analysis on flights originating from the 10 biggest airports (measured specifically by the number of flights during the Thanksgiving holiday), which are much more likely to factor into a traveler's plans.
...
ggplot(by_airport_delay,aes(x=fct_reorder(ORIGIN,delay),y=delay,fill=fct_rev(period))) + 
  ...
  labs(subtitle = '20 Before & Five Days During Thanksgiving Holiday') +
...
This again ties into the 2018 anamoly (don't worry, we'll address it shortly!).

Now we'll start tackling the juciest question aside from airline choice - *when* do you travel?

## Comparing Days & Times Of Travel

We will now deep dive into our data to analyze how airline delays look like leading up to and post Thanksgiving. This will help us ascertain whether there is a general trend, across the years, of airline delays and cancellations on specific days.
...
Continuing to deep dive into this issue, we found [a news article online talking specifically about the day and the year we were observing.](https://www.usatoday.com/story/travel/flights/todayinthesky/2018/11/25/snow-flights-canceled-storm-snarls-post-thanksgiving-sunday/2107525002/)
...
We will now answer a vital question: how has airline OTP (on-time performance) during the Thanksgiving holiday compared to airline OTP outside of the Thanksgiving holiday since the mergers of the major US airlines?
...
It's worth noting that more flights creates more dots here (there are about four times as many flights during the "Pre-Thanksgiving" period and the larger airlines have more flights overall), so we're looking for where the dots cluster, rather than how many dots there are overall in a given area. 
...
Very strange - while there's a peak at the end of the Thanksgiving holiday (the final day is that dreaded Thanksgiving Sunday), there's another peak about a week before Thanksgiving that's worse than that final Sunday, and much worse than the traditionally-dreaded Thanksgiving Wednesday. Perhaps people are leaving earlier than we thought.
...
When planning Thanksgiving holiday air travel, don't obsess about straying from the traditional travel periods - it doesn't seem that the Wednesday before Thanksgiving is much worse for delays than any other day, and the Sunday after isn't (usually) all that bad. Don't bother leaving a few days early to save yourself a travel headache - in fact, you may make it worse by doing that. If you want to beat the rush, you absoulutely want to leave earlier in the day - not earlier in the week. 

Don't bother avoiding ultra low-cost carriers like Spirit and Frontier - their performance actually *improves* during the holiday, contrary to what you might think. That's the most noticeable difference between airline OTP before vs. during the holiday.

Your choice of airport shouldn't matter too much - unless, of course, there's a storm.

The bottom line is this - don't fret about airline disasters during the Thanksgiving holiday, because in the years since the last major airline mergers went down in the US, Thanksgiving holiday travel has actually been *less* wrought with delays than pre-Thanksgiving travel. Even with a disastrous weather delay factored in (we mentioned this caveat earlier), Thanksgiving flight delays haven't been as bad as pre-Thanksgiving delays over the last few years. 

So, at the very least, we can pretty confidently say Thanksgiving travel isn't more delay-prone than non-Thanksgiving travel. We hope that next time you go to see your family and friends for the holiday, you can fly peacefully knowing you're just as likely to get there on time as you would be normally.

#####

# *TO DO: Fix color scales for the color-blind? (this was on assignment)*
...
# *TO DO: Organize GitHub as recommended here https://edav.info/project.html*
