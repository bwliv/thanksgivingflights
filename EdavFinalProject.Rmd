...
Ultra low-cost carrier Allegiant (G4) leads the pack in Thanksgivng delays, which isn't surprising considering its reputation (but wait, more on this in a second!). Regional airline Mesa Airlines comes next. Hawaiian Airlines has the fewest delays of any airline. 
...
ggplot(by_region_delay2,aes(x=region,y=delay)) + 
  ...
  ggtitle('Flight Delays By Region')+
...
ggplot(by_region_delay,aes(x=region,y=delay,fill=as.factor(year))) + 
  ...
  ylab('Average Delay (Minutes)') +
  labs(fill = 'Year') + 
  ggtitle('Thanksgiving Holiday Delays Grouped By Region & Year')+
  theme(plot.title = element_text(hjust = 0.5)) +
...
ggplot(by_flight_delay,aes(x=OP_UNIQUE_CARRIER,y=delay,fill=OP_UNIQUE_CARRIER)) + 
  ...
  ylab('Average Flight Delay (Minutes)') +
  labs(fill = 'Year') + 
...
by_airport_delay_ntg = alldates_top10 %>%  filter(period =="Non-Thanksgiving") %>% group_by(DEST,year,period) %>% summarize(delay = mean(ARR_DELAY_NEW,na.rm=TRUE))
...
alldates_top10_cancellations_Thanksgiving$DEST <- 
  ...
  alldates_top10_cancellations_Non_Thanksgiving <- alldates_top10[alldates_top10$period =="Non-Thanksgiving" &alldates_top10$year!=2018,] %>% group_by(DEST) %>% summarise(No_of_cnc = sum(CANCELLED,na.rm=TRUE)/80) %>% arrange(-No_of_cnc)
...
alldates_top10_delays_Non_Thanksgiving <- alldates_top10[alldates_top10$period =="Non-Thanksgiving" &alldates_top10$year!=2018,] %>% group_by(DEST) %>% summarise(No_of_delay = sum(if_else(ARR_DELAY_NEW>0,1,0),na.rm=TRUE)/80) %>% arrange(-No_of_delay)
